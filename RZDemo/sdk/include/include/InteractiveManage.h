...
public:
    ...
    /**
         ...
         */
    static TimerTaskID registTimedTask(const std::string moduleName, std::function<void(void)> taskFun,
                                       int timeInterval, bool Immediate);

    /**
         * 移除定时任务
         * @param taskId 任务ID
         */
    static void removeTimedTask(const TimerTaskID& taskId);
...
class AsyncQuit {
public:
    ~AsyncQuit() { InteractiveManage::stop(); }
};
}  // namespace rz
