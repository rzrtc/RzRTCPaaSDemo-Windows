...
//音频编码器码率控制项
enum AUDIO_BIT_RATE_CTRL_TYPE { AUDIO_BIT_RATE_CTRL_CBR, AUDIO_BIT_RATE_CTRL_VBR };
...
public:
    ...
    VideoData(const unsigned char* data, VIDEO_STREAM_TYPE streamType, VIDEO_PIXEL_FORMAT frameType, int w, int h,
              long ts) {
        if (data == nullptr)
            return;
        width = w;
   ...
   */
    VideoData(VIDEO_PIXEL_FORMAT fmt, uint32_t ts, uint32_t w, uint32_t h, bool mirror, uint8_t* plane[4],
              const uint32_t linesize[4]) {
   ...
   */
    ...
    void fillConvertData(VIDEO_PIXEL_FORMAT fmt, uint32_t w, uint32_t h, uint8_t* plane[4],
                         const uint32_t linesize[4]) {
        int lineCount = height / VideoPixelHeigth[format][0];
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (linesize[i] * lineC * VideoPixelHeigth[fmt][i] >
                this->lineSize[i] * lineCount * VideoPixelHeigth[format][i]) {
                delete[] this->Plane[i];
...
public:
   ...
   */
    AudioData(AUDIO_PCM_FORMAT fmt, uint32_t ts, uint32_t samplerate, uint32_t samplesize, uint32_t channel, bool AEC,
              bool NS, bool AGC, uint8_t* plane[MAX_AUDIO_CHANNEL], uint32_t linesize[MAX_AUDIO_CHANNEL]) {
   ...
   * @param dataLen    数据长度
   */
    AudioData(AUDIO_STREAM_TYPE streamType, AUDIO_PCM_FORMAT fmt, uint32_t samplerate, uint32_t channel, uint32_t ts,
   ...
   */
    ...
    void fillConvertData(AUDIO_PCM_FORMAT fmt, uint32_t samplerate, uint32_t samplesize, uint32_t channel,
                         uint8_t* plane[MAX_AUDIO_CHANNEL], uint32_t linesize[MAX_AUDIO_CHANNEL]) {
        format = fmt;
        channelCount = channel;
        sampleSize = samplesize;
