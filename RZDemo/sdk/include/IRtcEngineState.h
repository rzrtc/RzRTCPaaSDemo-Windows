...
namespace rz {
    ...
    enum CHANNEL_PROFILE_TYPE {
        ...
        CHANNEL_PROFILE_LIVE_BROADCASTING = 1,  //1：直播场景。该场景有主播和观众两种用户角色，可以通过 setClientRole 设置。主播可以发布和接收音视频流，观众直接接收流。适用于语聊房、视频直播、互动大班课等应用场景。
    };
    ...
    enum AUDIO_PROFILE_TYPE {
        AUDIO_PROFILE_DEFAULT = 0,                    //0: 默认值。直播场景下：16 KHz 采样率，单声道，编码码率最大值为 64 Kbps。通信场景下：16 KHz 采样率，单声道，编码码率最大值为 32 Kbps。
    ...
    struct VideoEncoderConfiguration {
        ...
        int minBitrate = 0;    //最小码率 kbps
        int frameRate = 0;     //帧率
        int minFrameRate = 0;  //最小帧率
        ORIENTATION_MODE orientationMode = ORIENTATION_MODE_ADAPTIVE; //视频输出模式(横屏、竖屏)
        VIDEO_MIRROR_MODE_TYPE mirrorMode = VIDEO_MIRROR_MODE_AUTO;   //镜像配置
    };
    ...
    //通信状态信息
    struct RtcStats {
        unsigned int duration = 0;            //通话时长（秒）。
        unsigned int txBytes = 0;             //发送字节数（bytes）。
        unsigned int rxBytes = 0;             //接收字节数（bytes）。
        unsigned int txAudioBytes = 0;        //发送音频字节数（bytes），累计值。
        unsigned int txVideoBytes = 0;        //发送视频字节数（bytes），累计值。
        unsigned int rxAudioBytes = 0;        //接收音频字节数（bytes），累计值。
        unsigned int rxVideoBytes = 0;        //接收视频字节数（bytes），累计值。
    ...
    struct RemoteAudioStats {
        ...
        int frozenRate = 0;              //加入频道后该远端用户音频卡顿的累计时长占音频总有效时长的百分比 (%)。音频有效时长是指远端用户加入频道后音频未被停止发送的时长。
    ...
    struct LocalVideoStats {
        ...
        unsigned short txPacketLossRate = 0;   //弱网对抗前本端到 RZ 边缘服务器的视频丢包率 (%)。
    ...
    enum CONNECTION_CHANGED_REASON_TYPE {
        ...
        CONNECTION_CHANGED_REJECTED_BY_SERVER = 7,          //7: 此用户被服务器拒绝, 一般是因为使用相同uid重复进入频,此时先进入频道的用户会收到 CONNECTION_STATE_FAILED, reason是CONNECTION_CHANGED_REJECTED_BY_SERVER
    ...
    enum USER_OFFLINE_REASON_TYPE {
        ...
        USER_OFFLINE_DROPPED = 1,              //1: 因过长时间收不到对方数据包，超时掉线。注意：由于 SDK 使用的是不可靠通道，也有可能对方主动离开本方没收到对方离开消息而误判为超时掉线。
...
#endif //PAASSDK_IRTCENGINESTATE_H
